...
/**
 * Taken from: https://github.com/kirkbushell/eloquence/commits/master/src/Behaviours/CamelCasing.php
 */
trait CamelCasing
{
    /**
    ...
    /**
     * Custom mapping to transform snake case to camel case, for example fields with numbers, for example;
     * 
     ...
     * in any case are converted to Str::snake, which is the defacto standard
    ...
    {
        return array_map('Str::snake', $this->hidden);
    }
    /**
     * Overloads the eloquent getDates method to ensure that date field declarations
     * can be made in camelCase but mapped to/from DB in Str::snake.
     *
     * @return array
     */
    public function getDates()
    {
        ...
        return array_map('Str::snake', $dates);
    }
    /**
    ...
    {
        // return key of camel case value
        if (in_array($key, $this->maps)) return array_search($key, $this->maps);

        return Str::snake($key);
    }
    /**
     * Because we are changing the case of keys and want to use camelCase throughout the application, whenever
     * we do isset checks we need to ensure that we check using Str::snake.
     *
     * @param $key
     * @return mixed
     */
    ...
    /**
     * Because we are changing the case of keys and want to use camelCase throughout the application, whenever
     * we do unset variables we need to ensure that we unset using Str::snake.
     *
     * @param $key
     * @return void
     */
